...
def main():
    ...
    even_better_draw_circles(rg.Point(50,50), 20, 30, 'blue', 10)
    window.update()
...
def even_better_draw_circles(point,radius,n,pencolor,penthickness):
    turtle = rg.SimpleTurtle()
    turtle.pen = rg.Pen(pencolor, penthickness)
    turtle.pen_up()
    #turtle.pen.color = rg.Pen.color(pencolor)
    #turtle.pen.thickness = rg.Pen.thickness(penthickness)
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)
